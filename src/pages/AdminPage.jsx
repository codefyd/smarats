...
function StalePlaylistsSection() {
  ...
  async function fetchStale() {
    setLoading(true)
    const { data, error } = await supabase.rpc('list_stale_playlists')
    setLoading(false)
    if (error) return alert('خطأ: ' + error.message)
    setStale(data || [])
  }
  ...
  async function deleteStale() {
    if (!confirm(`هل أنت متأكد من حذف ${stale.length} قائمة راكدة؟ هذا لا يمكن التراجع عنه.`)) return
    setDeleting(true)
    const { error } = await supabase.rpc('delete_stale_playlists')
    setDeleting(false)
    if (error) return alert('خطأ: ' + error.message)
    setStale([])
  ...
  return (
    <div className="card mt-6">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div>
          <h3 className="font-bold">القوائم الراكدة</h3>
          <p className="text-xs text-slate-500 mt-0.5">
            قوائم فيها عناصر، غير مرتبطة بأي شاشة، ولم تُستخدم منذ أكثر من 90 يوم
          </p>
        </div>
        <button onClick={fetchStale} disabled={loading} className="btn btn-secondary text-xs">
          {loading ? 'جاري الفحص...' : 'فحص القوائم الراكدة'}
        </button>
      </div>

      {stale !== null && (
        stale.length === 0 ? (
          <p className="text-sm text-green-600">✓ لا توجد قوائم راكدة</p>
        ) : (
          <div>
            <div className="space-y-2 max-h-64 overflow-y-auto mb-4">
              {stale.map(p => (
                <div key={p.id} className="flex items-center justify-between text-sm gap-2 p-2 rounded bg-slate-50">
                  <div className="min-w-0">
                    <div className="font-medium truncate">{p.name}</div>
                    <div className="text-xs text-slate-500">{p.items_count} عنصر · خامد {p.days_idle} يوم</div>
                  </div>
                </div>
              ))}
            </div>
            <button onClick={deleteStale} disabled={deleting} className="btn btn-danger text-xs">
              {deleting ? 'جاري الحذف...' : `حذف ${stale.length} قائمة راكدة`}
            </button>
          </div>
        )
      )}
